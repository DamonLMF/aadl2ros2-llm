...
# -*- coding: utf-8 -*-
"""
AADL to ROS2 conversion CLI tool
usage: python aadl2ros2_agent_cli.py -i ./example/fcc -f Flight_Controller.aadl -s Flight_Controller -o ./output -k your_api_key
"""
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="AADL to ROS2 conversion tool using multi-agent system",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
        Examples:
        python aadl2ros2_agent_cli.py -i ./example/fcc -f Flight_Controller.aadl -s Flight_Controller 
        -m ./mapping.csv -o ./output -k your_api_key
        """
    )
    ...
    # Run Phase 4: repeat until no dynamic errors (bounded by min/max iterations)
    repair_iter = 0
    max_repair_iters = 5
    repair_attempts = {}
    while True:
        repair_iter += 1
        console.print(
            f"[bold cyan]Dynamic test iteration {repair_iter}/{max_repair_iters}"
        )
        ...
        # No errors and no timing changes → clean run, stop only after reaching min iterations.
        ...
        if only_components:
            console.print("[bold yellow]Dynamic test found component errors. Regenerating component code...[/bold yellow]")
            ...
            console.print(
                f"[bold cyan]{repair_scope} components:[/bold cyan] {', '.join(repair_components)}"
            )
            component_error_context = "\n".join(
                f"- error_type={e.get('error_type')} component={e.get('component')} "
                f"message={e.get('exception_message')} "
                f"root_cause_analysis={e.get('root_cause_analysis')}"
                for e in errors
                if isinstance(e, dict) and e.get("component") in only_components
            )
            component_error_context += (
                "\n\n[REPAIR SCOPE]\n"
                f"Mode: {repair_scope}.\n"
                f"Regenerate only these components: {', '.join(repair_components)}.\n"
                "Keep existing topics, message types, QoS, CMake files, and public component interfaces stable. "
                "Make the smallest behavior fix needed for the reported errors."
            )
            component_error_context += ledger_rules
            updated_state = coder_agent(
                console,
                updated_state,
                only_components=repair_components,
                error_context=component_error_context,
            )
            for e in errors:
                if isinstance(e, dict) and e.get("component") in only_components:
        ...
        elif only_main_nodes:
            # Node errors: regenerate all components with error context as guidance, then re-iterate.
            console.print(
                "[bold yellow]Detected node errors. Regenerating components with error context and re-running...[/bold yellow]"
            )
            node_error_context = "\n".join(
                f"- error_type={e.get('error_type')} node={e.get('node')} "
                f"message={e.get('exception_message')} "
                f"root_cause_analysis={e.get('root_cause_analysis')}"
                for e in errors if e.get("node")
            )
            node_error_context += ledger_rules
            updated_state = coder_agent(
                console,
                updated_state,
                error_context=node_error_context,
            )
            for e in errors:
                if isinstance(e, dict) and e.get("node") in only_main_nodes:
                    sig = error_signature(e)
                    repair_attempts[sig] = repair_attempts.get(sig, 0) + 1
            continue
        else:
