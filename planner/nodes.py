...
def runtime_analysis_tool(console, state: SystemState) -> Dict:
    ...
    if not os.path.exists(node_log_path):
        ...
        state["runtime_analysis_warnings_by_component"] = {}
    ...
    state["runtime_analysis_warnings_by_component"] = analysis.get_coverage_warnings()
...
def dynamic_tester_tool(console, state: SystemState) -> Dict:
    ...
    command = [sys.executable, dynamic_test_script, "-p", output_dir, "--phase-runs", "3"]
    if bool(state.get("inject_virtual_io")):
        command.append("--inject-virtual-io")
    console.print(f"execute command: {command}")
    subprocess.run(command, check=True, cwd=os.getcwd())
    ...
    if node_errors.get("unparsed_colcon_build") and state.get("api_key"):
        parsed = llm_parse_colcon_build_errors(
            api_key=state["api_key"],
            xml_path=state["xml_file_path"],
            colcon_build_log=node_errors.get("colcon_build_log") or "",
            raw_blocks=node_errors.get("raw_blocks", []),
        )
        if parsed:
            level_1_errors.extend(parsed)
    ...
    elif not node_errors.get("unparsed_colcon_build"):
        # Phase 4.2: no level-1 parse errors — runtime analysis on dynamic test output
        runtime_analysis_tool(console, state)
        for comp, items in (state.get("runtime_analysis_warnings_by_component") or {}).items():
            warnings_by_component.setdefault(comp, []).extend(items or [])
...
def coder_agent(
    ...
    """
    Code generation agent (C++). Main node / test nodes are template-only; incremental regen supports
    ``only_components`` or ``only_other_codes``.
    """
    ros2_arch_file = state["ros2_arch_file"]
    # Run coder agent
    coder_agent_script = os.path.join(os.path.dirname(os.path.dirname(__file__)), "coder_agent.py")
